...
7. **Independent review before the athlete sees it**: spawn the `coach-advisor` agent
   (defined in `.claude/agents/coach-advisor.md`) with the draft plan, the baseline
   numbers, and the goal. This step is for the drafting session only — if you ARE the
   coach-advisor agent reading this, do not spawn another advisor; review it yourself. It verifies the numbers against the database itself and
